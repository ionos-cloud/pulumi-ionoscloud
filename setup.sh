...
if test ! -d "provider/cmd/pulumi-tfgen-ionoscloud"; then
  echo "Project already renamed, provider/cmd/pulumi-tfgen-ionoscloud not found"
  exit 1
fi
...
# ci-mgmt
replace .ci-mgmt.yaml "provider: ionoscloud" "provider: ${provider}"
replace .ci-mgmt.yaml "organization: pulumi" "organization: ${organization}"
make ci-mgmt

# Readme
# Delete README.md prelude up to the provider title line
provider_title_line_number=$(grep -n '# ionoscloud Resource Provider' README.md | cut -f1 -d:)
sed_inplace README.md "1,$((provider_title_line_number-1))d"
replace README.md "ionoscloud" "${provider}"
replace README.md "ionoscloud" "$(echo "${provider}" | awk '{print toupper(substr($0,1,1)) substr($0,2)}' || true)"
replace README.md "ionoscloud" "$(echo "${provider}" | awk '{print toupper($0)}' || true)"

# Remove bridge metadata
rm -f provider/cmd/pulumi-resource-ionoscloud/bridge-metadata.json

# Go modules & code
replace_go_src() {
  replace "${1}" github.com/pulumi/terraform-provider-ionoscloud "${upstream}"
  replace "${1}" "github\.com/ionos-cloud/pulumi-ionoscloud" "github\.com/${organization}/${repository}"
  replace "${1}" ionoscloud "${provider}"
}
replace provider/go.mod "github\.com/ionos-cloud/pulumi-ionoscloud" "github\.com/${organization}/${repository}"
replace provider/go.mod "github.com/pulumi/terraform-provider-ionoscloud v.*" "${upstream} latest"
replace_go_src provider/resources.go
replace_go_src provider/cmd/pulumi-tfgen-ionoscloud/main.go
replace_go_src provider/cmd/pulumi-resource-ionoscloud/main.go
if [[ "${provider}" != "ionoscloud" ]]; then # Don't fail on no-op rename
  mv provider/cmd/pulumi-tfgen-ionoscloud "provider/cmd/pulumi-tfgen-${provider}"
  mv provider/cmd/pulumi-resource-ionoscloud "provider/cmd/pulumi-resource-${provider}"
fi
# Resolve to a real upstream version in the go.mod
(cd provider && go get "${upstream}")
replace examples/go.mod "github\.com/ionos-cloud/pulumi-ionoscloud" "github\.com/${organization}/${repository}"
